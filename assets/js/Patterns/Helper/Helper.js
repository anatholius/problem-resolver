...
export const mapToObjectSimple = map => Object.fromEntries(map);
...
export const camelToArray = (string) => string.replace(/[\w]([A-Z])/g, (m) => m[0] + "_" + m[1]).toLowerCase().split('_');

export const arrayHasEvery = (arraySource, arrayTarget) => arrayTarget.every(v => arraySource.includes(v));
export const arrayHasAny = (arraySource, arrayTarget) => {
...
export const missingTables = (arraySource, toFind) => toFind.filter(e => !arraySource.includes(e));
...
export function populate(object, entity, data) {
    if (entity && data) {
        switch (entity) {
            case 'owner':
                break;
            case 'company':
                object.company = {
                    ...
                    nip: data['nip'],
                    regon: data['regon'],
                    foundingDate: this.formatDate(data['foundingDate']),
...
export function parseEntity(entity, data) {
    if (!data) {
        return null;
    }
    let parsed = {
    ...
    switch (entity) {
        ...
        case 'company': {
            ...
            for (let key in data['bankAccounts']) {
                if (data['bankAccounts'].hasOwnProperty(key)) {
                    bankAccounts.push(parseEntity('bankAccount', data['bankAccounts'][key]));
                }
            }
        ...
        case 'settlement': {
            ...
            if (data['book']) {
                parsed.book = parseEntity('book', data['book']);
            }
            break;
        }
        case 'document': {
            parsed.type = data['type'];
            // parsed.invoice = parseEntity('invoice', data['invoice']);
            // parsed.intproof = parseEntity('intproof', data['intproof']);
            parsed.col1 = data['col1'];
            parsed.col2 = data['col2'];
            parsed.col3 = data['col3'];
            parsed.col4 = data['col4'];
            parsed.col5 = data['col5'];
            parsed.col6 = parseEntity('economicEvent', data['col6']);
            ...
            if (data['settlement']) {
                parsed.settlement = parseEntity('settlement', data['settlement']);
            }
            break;
        }
        case 'invoice': {
            ...
            for (let key in data['invoiceItems']) {
                if (data['invoiceItems'].hasOwnProperty(key)) {
                    items.push(parseEntity('invoiceItem', data['invoiceItems'][key]));
                }
            }
            ...
            parsed.economicEvent = parseEntity('economicEvent', data['economicEvent']);
            break;
        }
        case 'invoiceItem': {
            parsed.pos = data['pos'];
            ...
            parsed.economicOperation = parseEntity('economicOperation', data['economicOperation']);
            break;
        }
        case 'intproof': {
            parsed.city = data['city'];
            ...
            for (let key in data['intproofItems']) {
                if (data['intproofItems'].hasOwnProperty(key)) {
                    items.push(parseEntity('intproofItem', data['intproofItems'][key]));
                }
            }
        ...
        case 'economicEvent': {
            ...
            for (let key in data['economicOperations']) {
                if (data['economicOperations'].hasOwnProperty(key)) {
                    economicOperations.push(parseEntity('economicOperation', data['economicOperations'][key]));
                }
            }
 ...
 * @private
 */
export function printVignette(data) {
    return (data && data.nip) && (
        <React.Fragment>
            <b>{data.companyName}{data.isPartnership ? ' ' + parseSuffix(data.partnership) : ''}</b><br/>
 ...
 * @returns {string}
 */
export function parseAddress(data, target = 'kpir') {
    if (data) {
        if (target === 'vignette') {
...
export function parseSuffix(suffixValue) {
    const suffixes = {
        zoo: 'Sp. z o.o.',
        sa: 'S.A.',
    };
...
export function fullName(company) {
    return company.companyName + (company.isPartnership ? ' ' + parseSuffix(company.partnership) : '');
}

export function formatItemTitle(operation, date) {
    if (operation) {
        const title = ['description', 'title', 'period'].map(field => {
            ...
            if (operation[field] !== '' && operation[field] !== null) {
                result = field !== 'period' ? ' ' + operation[field] : ' - ' +
                    formatPeriod(operation[field], date)
                ;
            } else {
                result = '';
            }
 ...
 * @private
 */
export function formatOptions(fieldName, data, withNoSelect = true, conditions = [], mergeFields = []) {
            ...
            return [
                ...
                ...data.filter(option => {
                    ...
                    if (fieldName === 'companyName') {
                        optionValue = option[fieldName] + (option['isPartnership'] ? ' ' + parseSuffix(option['partnership']) : '');
                    } else {
 ...
 * @returns {null|*}
 */
export function getArrayElementById(id, array, childFieldName = null) {
    if (id) {
        if (childFieldName !== null) {
            ...
            const index = getIndexOfValue('id', id, array);
...
export function getIndexOfElementArrayById(id, array) {
...
export function getIndexOfValue(fieldName, fieldValue, array) {
    return array.findIndex(item => String(item[fieldName]) === String(fieldValue));
}

export function getIndexOfField(fieldName, fieldValue, array) {
    return array.findIndex(item => String(item[fieldName]) === String(fieldValue));
}
 ...
 * @returns {*}
 */
export function findArrayElementByFieldValue(fieldName, fieldValue, array) {
    const index = getIndexOfValue(fieldName, fieldValue, array);
    return index !== -1 ? array[index] : null;
}
 ...
 * @returns {null|*}
 */
export function findArrayElementByFieldDate(fieldName, date, array) {
    if (date) {
        for (let key in array) {
            if (array.hasOwnProperty(key)) {
...
export function formatPeriod(period, date) {
    return formatDate(date, 'invoicePeriod' + period)
}
 ...
 */
export function formatDate(date, type, changeYear = 0, changeMonth = 0) {
    if (!date) {
        return '';
    }
...
export function actionsEntity(actionName) {
    console.log(actionName);
    return actions[actions.findIndex(item => item.action === actionName)].entity;
}

export function entitysAction(entityName) {
    return actions[actions.findIndex(item => item.entity === entityName)].action;
}

export function entitysCollection(entityName) {
    return actions[actions.findIndex(item => item.entity === entityName)].collection;
}

export function _handleComaUnitPrice(e) {
...
export function comparePenny(document, col, showAsObject = false) {
